...
public class QnAController {
	...
	@ApiOperation(value = "답변 작성", notes = "QnA에 대한 답변을 작성한다. 그리고 DB수정 성공여부에 따라 'success' 또는 'fail' 문자열을 반환한다.", response = String.class)
	@PutMapping("/{articleno}")
	public ResponseEntity<String> writeAnswer(@PathVariable("articleno") @ApiParam(value = "수정할 글정보.", required = true) int articleno) throws Exception {
		logger.info("writeAnswer - 호출");
		
		if (qnaService.writeAnswer(articleno)) {
			return new ResponseEntity<String>(SUCCESS, HttpStatus.OK);
		}
		return new ResponseEntity<String>(FAIL, HttpStatus.OK);
	}
