CREATE DATABASE  IF NOT EXISTS `ssafyweb` /*!40100 DEFAULT CHARACTER SET utf8 */ /*!80016 DEFAULT ENCRYPTION='N' */;
USE `ssafyweb`;
...
CREATE TABLE `datacomment` (
  ...
  CONSTRAINT `datacomment_ibfk_1` FOREIGN KEY (`articleno`) REFERENCES `freeboard` (`articleno`)
...
CREATE TABLE `localboard` (
  ...
  `regtime` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`articleno`),
  KEY `areaboard_to_member_fk` (`userid`),
...
CREATE TABLE `sharecomment` (
  ...
  PRIMARY KEY (`commentno`),
  KEY `articleno` (`articleno`),
  CONSTRAINT `sharecomment_ibfk_1` FOREIGN KEY (`articleno`) REFERENCES `freeboard` (`articleno`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3;
/*!40101 SET character_set_client = @saved_cs_client */;
...
-- Dump completed on 2021-11-24 21:19:24
