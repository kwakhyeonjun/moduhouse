<template>
  <v-app>
    <v-list>
      <v-list-item-title><h2>👨‍👦‍👦지역게시판</h2></v-list-item-title>
      <v-divider></v-divider>
...
<script>
import { getArticleLocalBoard, deleteArticleLocalBoard } from "@/api/board";
import { mapState } from "vuex";
const memberStore = "memberStore";
import CommentList from "./child/LocalCommentList.vue";
...
export default {
  ...
  methods: {
    listArticle() {
      this.$router.push({ name: "LocalBoard" });
    },
    moveModifyArticle() {
      this.$router.replace({
    ...
    removeArticle() {
      if (confirm("정말로 삭제?")) {
        deleteArticleLocalBoard(this.article.articleno, () => {
          this.$router.push({ name: "LocalBoard" });
        });
      }
    },
