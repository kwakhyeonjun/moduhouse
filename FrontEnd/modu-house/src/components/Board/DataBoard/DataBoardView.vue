<template>
  <v-app>
    <v-list>
      <v-list-item-title><h2>👨‍👦‍👦자료게시판</h2></v-list-item-title>
      <v-divider></v-divider>
...
<script>
import { getArticleDataBoard, deleteArticleDataBoard } from "@/api/board";
import { mapState } from "vuex";
const memberStore = "memberStore";
import CommentList from "./child/DataCommentList.vue";
...
export default {
  ...
  methods: {
    ...
    removeArticle() {
      if (confirm("정말로 삭제?")) {
        deleteArticleDataBoard(this.article.articleno, () => {
          this.$router.push({ name: "DataBoard" });
        });
      }
    },
